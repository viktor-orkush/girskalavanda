...
function glav_noindex_mphb_booking_pages($robots)
{
    ...
    if ($has_booking_params || is_page($booking_slugs) || is_page($exclude_ids) || is_post_type_archive('mphb_room_type')) {
        $robots['noindex'] = true;
...
/**
 * Quick localization for MPHB strings appearing in SERP/Frontend.
 * Avoids installing heavy translation plugins like Loco Translate.
 */
add_filter('gettext', 'glav_translate_mphb_strings', 20, 3);
function glav_translate_mphb_strings($translated_text, $text, $domain)
{
...
function glav_seo_document_title_parts($title)
{
    ...
    if (is_front_page() || is_page_template('page-home.php')) {
        ...
        $title['title'] = 'Гарячий чан у Східниці | Оренда чану в комплексі Гірська Лаванда';
        unset($title['site']);
    } elseif (is_page('rooms')) {
        $title['title'] = 'Апартаменти в Східниці | Оренда номерів у Гірській Лаванді';
        unset($title['site']);
    } elseif (is_page('gallery')) {
        $title['title'] = 'Фото комплексу Гірська Лаванда | Номери, Баня, Чан';
        unset($title['site']);
    } elseif (is_page('contact')) {
        $title['title'] = 'Контакти | Бронювання апартаментів Гірська Лаванда';
        unset($title['site']);
    } elseif (is_singular('mphb_room_type')) {
        $title['title'] = get_the_title() . ' | Оренда апартаментів у Східниці';
        unset($title['site']);
    }
...
{
    // Priority: Manual excerpt from WP editor
    ...
    if (is_page('gallery')) {
        return 'Фотогалерея комплексу Гірська Лаванда в Східниці — номери, апартаменти, баня, чан та краєвиди Східниці.';
    }
    if (is_page('contact')) {
    ...
    if (is_singular('mphb_room_type')) {
        ...
        $parts = array_filter([
            $capacity ? 'проживання для ' . $capacity . ' осіб' : '',
            $size ? 'площа ' . $size . ' м²' : '',
            $price ? 'від ' . $price . ' ₴/ніч' : '',
        ]);
        $details = $parts ? implode(', ', $parts) . '. ' : '';
        return 'Забронюйте ' . mb_strtolower(get_the_title()) . ' у комплексі Гірська Лаванда (Східниця). ' . $details . 'Відпочинок у Східниці з баньою та гарячим чаном.';
    }
...
function glav_schema_faq()
{
    ...
    if (is_front_page() || is_page_template('page-home.php')) {
        $faqs = [
            [
                'q' => 'Де розташований комплекс Гірська Лаванда?',
                'a' => 'Ми знаходимось у мальовничій Східниці, в тихому місці біля лісу, неподалік від популярних мінеральних джерел. Це ідеальне місце для тих, хто шукає затишок та чисте гірське повітря.'
            ],
            [
                'q' => 'Чи можна у вас зняти апартаменти з власною кухнею?',
                'a' => 'Так, наші апартаменти та котеджі обладнані повноцінними кухнями з усім необхідним (плита, посуд, холодильник) для комфортного тривалого проживання та самостійного приготування їжі.'
            ],
            [
                'q' => 'Які послуги оздоровлення пропонує ваш комплекс?',
                'a' => 'У нас ви можете замовити справжню баню на дровах, хамам та гарячий чан просто неба з видом на гори. Це чудовий спосіб відновити сили після прогулянок Східницею.'
            ],
            [
                'q' => 'Чи є у вас парковка для гостей?',
                'a' => 'Так, на території комплексу Гірська Лаванда діє безкоштовна охоронювана парковка для всіх наших гостей.'
            ],
            [
                'q' => 'Як далеко ви знаходитесь від мінеральних джерел Східниці?',
                'a' => 'Більшість популярних мінеральних джерел (Нафтуся, №18, №2с) розташовані в межах 10-15 хвилин легкої прогулянки від нашого комплексу.'
            ]
        ];
    } elseif (is_page('banya')) {
        $faqs = [
            [
                'q' => 'Скільки коштує баня у Східниці?',
                'a' => 'Вартість оренди нашої приватної бані починається від 2 500 ₴ за сеанс (мінімум 2 години). У вартість входить парна на дровах, кімната відпочинку та закрита територія.'
            ],
            [
        ...
        $faqs = [
            [
                'q' => 'Скільки коштує чан у Східниці?',
            ...
            [
                ...
                'a' => 'Так! Купання в гарячому чані під відкритим небом взимку в оточенні снігу — це один із найкращих видів релаксу у Східниці.'
            ],
            [
...
/**
 * [gl_faq_home] — секція частих питань для головної сторінки
 */
add_shortcode('gl_faq_home', 'glav_sc_faq_home');
function glav_sc_faq_home($atts)
{
    $faqs = [
        [
            'q' => 'Де розташований комплекс Гірська Лаванда?',
    ...
    ob_start();
    ?>
    <section class="gl-section gl-faq-home gl-section--white" id="home-faq">
        <div class="gl-container">
            ...
            <div class="gl-faq-home__list gl-animate gl-animate--delay-1">
...
function glav_customize_header_menu($items, $args)
{
    ...
    $is_hidden_item = function ($item) {
        ...
        foreach ($hidden_slugs as $slug) {
            if (
                stripos($item, 'href="' . home_url($slug)) !== false
                || stripos($item, "href='" . home_url($slug)) !== false
            )
                return true;
        }
    ...
    if (in_array($args->theme_location, ['mobile_menu', 'ast-mobile-menu', 'primary_mobile', 'off-canvas'], true)) {
        ...
        usort($menu_items, function ($a, $b) use ($mobile_order) {
            ...
            foreach ($mobile_order as $key => $score) {
                if (stripos($a, $key) !== false) {
                    $a_score = min($a_score, $score);
                }
                if (stripos($b, $key) !== false) {
                    $b_score = min($b_score, $score);
                }
            }
    ...
    if ($args->theme_location === 'primary') {
        ...
        // SEO: Clean alt text (remove "логотип" if present in site name)
        $clean_alt = str_ireplace('логотип', '', $site_name);
        $clean_alt = trim(preg_replace('/\s+/', ' ', $clean_alt));
        ...
        $logo_html = '<li class="menu-item menu-item-logo">'
            ...
            . ($logo_url ? '<div class="gl-menu-logo-wrap"><img src="' . esc_url($logo_url) . '" alt="' . esc_attr($clean_alt) . '" class="gl-menu-logo"></div>' : '')
        ...
        <div class="gl-container">
            ...
            <div class="gl-advantages__grid">
                <?php foreach ($advantages as $i => $adv): ?>
                    <div
                        class="gl-advantage-item gl-animate gl-animate--scale gl-animate--delay-<?php echo min($i + 1, 5); ?>">
        ...
        <div class="gl-container">
            <div class="gl-contacts__grid">
                <div class="gl-contacts__info gl-animate">
                    ...
                    <p class="gl-contacts__subtitle">Ми знаходимось у мальовничому містечку Східниця. Будемо раді вас
                        прийняти!</p>
                    ...
                    <div class="gl-contacts__items">
                        <div class="gl-contact-item">
                            ...
                            <div>
                                ...
                                <div class="gl-contact-item__value">Львівська обл. пгт Східниця,<br>
                                    вулиця Шевченко, 162-Н</div>
                            </div>
                        </div>
                        <?php
                        $phone = get_theme_mod('gl_phone', '');
                        ...
                        if ($phone): ?>
                            <div class="gl-contact-item">
                                ...
                                <div>
                                    ...
                                    <div class="gl-contact-item__value"><a
                                            href="tel:<?php echo esc_attr($phone); ?>"><?php echo esc_html($phone_disp ?: $phone); ?></a>
                                    </div>
                                </div>
                            </div>
                        <?php endif; ?>
                        <div class="gl-contact-item">
                            ...
                            <div>
                                ...
                                <div class="gl-contact-item__value"><a
                                        href="mailto:info@girskalavanda.com">info@girskalavanda.com</a></div>
                            </div>
                        </div>
                        <div class="gl-contact-item">
                            <div class="gl-contact-item__icon">🕐</div>
                            <div>
                                ...
                                <div class="gl-contact-item__value">Check-in від 14:00 &nbsp;·&nbsp; Check-out до 12:00
                                </div>
                            </div>
                        </div>
                        <?php
                            ...
                            <div class="gl-contact-item">
                                ...
                                <div>
                                    ...
                                    <div class="gl-contact-item__value">
                                        <?php if ($instagram): ?><a href="<?php echo esc_url($instagram); ?>"
                                                target="_blank" rel="noopener">Instagram</a><?php endif; ?>
                                        <?php if ($instagram && $facebook): ?>&nbsp;·&nbsp;<?php endif; ?>
                                        <?php if ($facebook): ?><a href="<?php echo esc_url($facebook); ?>" target="_blank"
                ...
                <div class="gl-contacts__map gl-animate gl-animate--delay-2">
                    ...
                    <iframe src="<?php echo esc_url($maps_embed_url); ?>" loading="lazy"
                ...
                <?php else: ?>
                    ...
                    foreach ($placeholders as $i => $ph): ?>
                        <div class="gl-gallery__item">
                            <div class="gl-gallery__placeholder"
                                style="background:<?php echo $gradients[$i]; ?>; font-size: 48px;">
                                <?php echo $ph; ?>
                            </div>
                        </div>
                    <?php endforeach; ?>
...
function glav_customize_register($wp_customize)
{
    ...
    $wp_customize->add_setting('gl_hero_subtitle', [
        'default' => 'Заміський комплекс · Східниця',
    ...
    <div class="gl-contact-fab" id="gl-contact-fab">
        <div class="gl-contact-fab__channels">
            <?php foreach ($channels as $i => $ch): ?>
                <a class="gl-contact-fab__channel" href="<?php echo esc_url($ch['url']); ?>" target="_blank"
                    rel="noopener noreferrer" aria-label="<?php echo esc_attr($ch['label']); ?>"
        ...
        <button class="gl-contact-fab__toggle" id="gl-contact-fab-toggle" aria-label="Контакти" title="Зв'язатися з нами">
            <svg class="gl-contact-fab__icon-open" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"
                stroke-linecap="round" stroke-linejoin="round">
                <path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z" />
            </svg>
            <svg class="gl-contact-fab__icon-close" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"
                stroke-linecap="round" stroke-linejoin="round">
